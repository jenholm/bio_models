...
<!-- TODO: Verify word count is within target (250 words) -->
<!-- TODO: Confirm "simulation-consistent" language per claim_discipline.md -->
...
<!-- TODO: Need citation for predator removal field studies (Getz 1974 already in bib) -->
...
<!-- TODO: Confirm this boundary language matches README.md -->
...
<!-- TODO: Consider adding brief description of each null model's equations -->
...
## 3.4 External Empirical Proxy Design
...
<!-- TODO: These proxy variables need clearer motivation in context of HSAP -->

# 4. Results

<!-- TODO: Verify all numbers against frozen data in results/freezes/ -->
...
**Well-mixed population.** The model assumes a well-mixed population without spatial structure or dispersal. This is a deliberate simplification to isolate endocrine-behavior mechanisms. Spatial extension is a natural next step.

**Linear endocrine model.** The endocrine update equations are linear combinations of environmental and social factors. Real endocrine systems involve nonlinear feedback, receptor dynamics, and individual genetic variation. The linear model captures first-order effects only.
...
**Figure 6.** Factorial sweep heatmap showing mean final population across predator pressure and disease pressure, separated by space constraint (low=0.2, high=0.8). Space constraint is the dominant population regulator: low constraint produces populations of 99–143 regardless of other parameters, while high constraint produces 7.7–99 with strong interactions between predator and disease pressure. This is a controlled parameter sweep, not an empirically calibrated phase diagram.
...
See `arxiv/supplementary/` for:
- **SM1**: Model equations (`model_equations.md`)
- **SM2**: Parameter tables (`parameter_table.md`)
- **SM3**: Null model definitions (`null_models.md`)
- **SM4**: Sensitivity analysis methods (`sensitivity_methods.md`) — note: formal Sobol analysis was not completed for this release
- **SM5**: Genetic algorithm methods (`GA_methods.md`)

Complete results tables in `paper_working/tables/` (CSV) and `paper_working/tables_md/` (Markdown):
...
All 61 scenarios are defined in `src/hsap/scenarios.py` with frozen parameter values. The result freeze is archived in `results/freezes/hsap_1970_seed_freeze_20260714/` with SHA-256 hashes for code tarballs. The model code is available in `src/hsap/` with full test coverage in `tests/`.
